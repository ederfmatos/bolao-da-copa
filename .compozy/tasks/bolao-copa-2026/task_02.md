...
- [x] Enable Google provider in Supabase Auth settings; record Client ID and Secret.
- [x] Add authorized redirect URIs in Google Cloud Console for dev and prod.
- [x] Verify `handle_new_user` trigger maps `raw_user_meta_data` correctly to `profiles`.
- [x] Manually test sign-in flow end-to-end in a local/staging environment.
- [x] Document setup steps in `docs/auth-setup.md`.
